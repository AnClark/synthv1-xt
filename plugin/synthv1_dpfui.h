...
private:
    ...
    ImFont *fFontMain, *fFontRadioButton, *fFontKnobValue;
    ResizeHandle fResizeHandle;

    // -----------------------------------------------------
    // UI helpers - create compound widgets
    ...
    inline constexpr std::string _trimAfterDoubleHash(const std::string& str) {
        size_t pos = str.find("##");
...
// NOTE: This is for official Qt-based UI only. Unused by synthv1-XT.
//
...
public:
	...
	// Constructor.
	synthv1_dpfui(synthv1_dpf *pSynth, SynthV1PluginUI *pluginUiInterface);

	// Accessors.
	void write_function(synthv1::ParamIndex index, float fValue) const;

private:
	SynthV1PluginUI *m_plugin_ui;
};
